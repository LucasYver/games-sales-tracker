...
async function main(): Promise<void> {
  ...
  try {
    if (slug) {
      ...
      logger.log(`Backfilling "${game.name}" (followers)…`);
    ...
    logger.log('Starting full games-popularity backfill (followers)…');
    const result = await ingestion.syncAllGamesPopularity({
      fullHistory: true,
    });
    logger.log(
      `Backfill complete: ${result.processed} game(s) processed, ` +
        `${result.followers} with followers, ${result.failed} error(s).`,
    );
  } finally {
    await app.close();
  }
