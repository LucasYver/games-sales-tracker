...
// A dated sales-related figure for a game on one platform, coming from a
// single source. Officially-declared totals, press announcements, Wikipedia
...
// trusted source's weight, purely informational).
//
// Calibration always picks the most recent milestone (by `reportedAt`) per
// platform, regardless of source. `confidenceScore` does NOT influence the
...
export class Milestone {
  ...
  // calibration, the per-platform breakdown headline and discrepancy
  // evaluation — they exist purely as an informational signal.
  ...
  // refuse to re-insert the same figure (matched by gameId + platform +
  // source + sourceUrl + units + reportedAt). All reads exclude rejected
  // rows.
  @Index()
  @Column({ type: 'timestamptz', nullable: true })
