...
// month (the month's last value). Followers are a slowly-rising running total,
// so keeping the end-of-month state preserves the trend.
...
export function FollowersHistoryChart({ followersHistory }: Props) {
  ...
  const { data, aggregated } = useMemo(() => {
    ...
    // Followers are cumulative-ish: keep the last point of each UTC month so
    // the line still represents the running total at end of month.
  ...
  if (allPoints.length === 1) {
    return (
      <div className="text-muted-foreground p-6 text-sm">
        Only one Steam follower snapshot so far (
            ...
            content={
              <ChartTooltipContent
                ...
                formatter={(value, name) => {
                  if (name === 'current') {
                    return [
                      formatFollowers(value as number),
                      aggregated ? 'Followers (end of month)' : 'Followers',
                    ];
                  }
                  return null;
                }}
