...
// month (the month's peak): viewers are an instantaneous count like CCU, so
// the monthly high is the meaningful zoomed-out value.
...
export function TwitchViewersHistoryChart({ twitchViewersHistory }: Props) {
  ...
  if (allPoints.length === 0) {
    return (
      <p className="text-muted-foreground p-6 text-sm">
        No Twitch viewer snapshots yet. They are polled hourly for every tracked
        game (daily peak kept). History is forward-only — it starts when polling
        began, so the chart fills in over time.
      </p>
    );
  }
  ...
  if (allPoints.length === 1) {
    return (
      <div className="text-muted-foreground p-6 text-sm">
        Only one Twitch viewer snapshot so far (
        {allPoints[0].current.toLocaleString()} on {formatDay(allPoints[0].t)}).
        The chart will appear once more points are captured.
      </div>
    );
  }
            ...
            content={
              <ChartTooltipContent
                ...
                formatter={(value, name) => {
                  if (name === 'current') {
                    return [
                      formatViewers(value as number),
                      aggregated ? 'Monthly peak viewers' : 'Twitch viewers',
                    ];
                  }
                  return null;
                }}
